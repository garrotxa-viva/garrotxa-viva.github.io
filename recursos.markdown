---
layout: default
title: Recursos - Garrotxa Viva
---

<main class="content-box recursos-content" markdown="1">

# Recursos

- [Els 'manneken pis' de la vall <span class="source">El Punt Avui</span>](https://www.elpuntavui.cat/societat/article/12-infraestructures/2554574-els-manneken-pis-de-la-vall.html)

- [La Vall d'en Bas exigeix un informe de l'ACA que garanteixi que la variant d'Olot no afectarà l'aqüífer que travessa <span class="source">La Vanguardia</span>](https://www.lavanguardia.com/local/girona/20250613/10786118/vall-d-bas-exige-informe-aca-garantice-variante-olot-afectara-acuifero-atraviesa.html)

- [Junts diu que la variant no respon a les necessitats reals de la Vall d'en Bas <span class="source">El Punt Avui</span>](https://www.elpuntavui.cat/societat/article/12-infraestructures/2583383-junts-diu-que-la-variant-no-respon-a-les-necessitats-reals-de-la-vall-d-en-bas.html)

- [La Finestra: Neix la plataforma Garrotxa Viva <span class="source">Olot TV</span>](https://youtu.be/6pZdJ-uSd_U?si=D26qzMOGagNrPGq8)

- [Així serà la variant d'Olot i les Preses <span class="source">La Comarca</span>](https://www.lacomarca.cat/noticia/257266/aixi-sera-la-variant-dolot-i-les-preses)

- [La Comarca d'Olot - Edició digital <span class="source">iQuiosc</span>](https://www.iquiosc.cat/publicacions/la-comarca-dolot/2287)

- [Diverses entitats alerten de l'impacte de la variant de les Preses a l'aqüífer de la Vall d'en Bas <span class="source">Directa</span>](https://directa.cat/entitats-ecologistes-alerten-de-limpacte-de-la-variant-de-les-preses-a-laquifer-de-la-vall-den-bas/)

- [Notícies sobre la variant de les Preses <span class="source">El Setembre</span>](https://www.elsetembre.cat/noticies/etiqueta/variant+de+les+Preses)

- [Preocupació entre els pagesos pel futur de l'aqüífer de la Vall d'en Bas <span class="source">3Cat</span>](https://www.3cat.cat/3cat/preocupacio-entre-els-pagesos-pel-futur-de-laquifer-de-la-vall-den-bas/video/6356636/)

- [Els comuns insten el govern a protegir l'aqüífer <span class="source">Diari de Girona</span>](https://www.diaridegirona.cat/comarques/2025/09/10/els-comuns-insten-govern-protegir-121425244.html)

</main>
